    ...
    public int calPoints(String[] ops) {
        ...
        for (int i = 0; i < ops.length; i++) {
            if (ops[i].equalsIgnoreCase("D")) {
                ...
                stackList.push(stackList.peek() * 2);
                ...
                top = stackList.pop();
                newValue = top + stackList.peek();
                stackList.push(top);
                stackList.push(newValue);
            } else {
                stackList.push(Integer.parseInt(ops[i]));
            }
        }
        // 遍历栈 计算总值
        int result = 0;
